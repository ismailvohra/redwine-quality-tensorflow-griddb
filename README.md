...
const colConInfo = new griddb.ContainerInfo({
    'name': "RedWine",
    'columnInfoList': [
      ["fixed acidity", griddb.Type.DOUBLE],
      ["volatile acidity", griddb.Type.DOUBLE],
      ["citric acid", griddb.Type.DOUBLE],
      ["residual sugar", griddb.Type.DOUBLE],
      ["chlorides", griddb.Type.DOUBLE],
      ["free sulfur dioxide", griddb.Type.INTEGER],
      ["total sulfur dioxide", griddb.Type.INTEGER],
  ...
  .on('data', (row) => {
    ...
    store.putContainer(conInfo, false)
       ...
       .then(col => {
           ...
           col.put([row['fixed acidity'], row['volatile acidity'], row['citric acid'], row['residual sugar'], row['chlorides'], row['free sulfur dioxide'], row['total sulfur dioxide'], row['density'], row['pH'], row['sulphates'], row['alcohol'], row['quality']]);
           return col;
       })
  })
  .on('end', () => {
...
We will now fetch the data from GridDB and perform our Exploratory Data Analysis on it. After which we can move to preprocessing and modelling.

```javascript
// Fetching Code <Incomplete>
var getData;
var lst = []
store.getContainer("RedWine")
       .then(ts => {
       getData = ts;
